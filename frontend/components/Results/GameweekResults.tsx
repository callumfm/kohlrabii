...
interface GameweekResultsProps {
  matches: schemas['FixtureForecastRead'][]
  className?: string
}

export function GameweekResults({ matches, className }: GameweekResultsProps) {
  if (matches.length === 0) {
    return null
  }
        ...
        <CardContent className="p-0">
          <div className="divide-y">
            {matches.map((match, index) => {
              ...
              const homeGoals = Math.round(match.home_goals_for)
              const awayGoals = Math.round(match.away_goals_for)
              ...
              return (
                ...
                >
                  ...
                  <div className="flex-1 flex justify-end items-center gap-3 min-w-[150px]">
                    <span className="font-semibold text-base truncate">{match.fixture.home_team}</span>
                    <div className="w-8 h-8 relative flex-shrink-0">
                      ...
                      <div className="w-full h-full flex items-center justify-center rounded-full bg-muted">
                        <span className="text-[10px]">
                          {match.fixture.home_team.substring(0, 3).toUpperCase()}
                        </span>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="flex-1 flex items-center gap-3 min-w-[150px]">
                    <div className="w-8 h-8 relative flex-shrink-0">
                      ...
                      <div className="w-full h-full flex items-center justify-center rounded-full bg-muted">
                        <span className="text-[10px]">
                          {match.fixture.away_team.substring(0, 3).toUpperCase()}
                        </span>
                      </div>
                    </div>
                    <span className="font-semibold text-base truncate">{match.fixture.away_team}</span>
                  </div>
                </div>
              )
            })}
